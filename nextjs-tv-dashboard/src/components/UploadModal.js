...
export default function UploadModal({ open, onClose, onPlaylistAdded }) {
  ...
  const handleSaveUploadedPlaylist = async () => {
    if (!m3u8Content.trim()) {
    ...
    try {
      ...
      const playlistInfo = {
        ...
        liveChannels,
        vodChannels,
        type: selectedFormat === 'auto' ? 'M3U' : selectedFormat.toUpperCase(),
        createdAt: existingPlaylist?.createdAt || new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        status: 'active',
        source: 'file',
              ...
              <CardContent>
                <Stack spacing={3}>
                  ...
                  {showPreview && previewChannels.length > 0 && (
                    <Button
                      variant="contained"
                      onClick={handleSaveUploadedPlaylist}
            ...
            <Card sx={{ mt: 3 }}>
              <CardContent>
                <Typography variant="h6" sx={{ mb: 2 }}>
                  Preview - {previewChannels.length} canais encontrados
                </Typography>

                <Grid container spacing={2}>
                  {previewChannels.slice(0, 8).map((channel, index) => (
                ...
                {previewChannels.length > 8 && (
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 2, textAlign: 'center' }}>
                    ... e mais {previewChannels.length - 8} canais
                  </Typography>
                )}
              </CardContent>
            </Card>
